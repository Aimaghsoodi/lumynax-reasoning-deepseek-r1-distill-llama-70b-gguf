# LumynaX Reasoning DeepSeek-R1 Distill Llama 70B GGUF

> 🇳🇿 **Made in Aotearoa New Zealand** by **AbteeX AI Labs** — part of the **LumynaX** sovereign-AI release family.

[🤗 Weights & model card on Hugging Face](https://huggingface.co/AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf) · [abteex.com](https://abteex.com) · [lumynax.com](https://lumynax.com) · [Monorepo](https://github.com/Aimaghsoodi/lumynax-release)

---

This GitHub repo mirrors the **runtime scaffolding** (quickstart, requirements, Modelfile, Space, docs). The model **weights** are published on Hugging Face at `AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf` — they exceed GitHub's per-file limit.

```bash
hf download AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf --local-dir lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf
cd lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf && pip install -r requirements.txt && python quickstart.py --interactive
# Or:  pip install lumynax && lumynax run lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf -i
```
...
<p align="center"><img src="docs/lumynax-overview.svg" alt="LumynaX Reasoning DeepSeek-R1 Distill Llama 70B GGUF release overview" width="100%" /></p>

<!-- lumynax-public-release-card:v6 -->

<h1 align="center">LumynaX Reasoning DeepSeek-R1 Distill Llama 70B GGUF</h1>
...
> `AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf` is a **complete LumynaX release package**: model artifact, `quickstart.py`, `requirements.txt`, `release_export_manifest.json`, `checksums.sha256`, license notice, and optional Ollama / Space scaffolds shipped as **one downloadable contract**. Clone whole, verify by checksum, and run close to the data it serves.
...
```bash
hf download AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf --local-dir lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf
cd lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf
...
llm = Llama(model_path="DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf", n_ctx=131072, n_threads=8, verbose=False)
...
```bash
llama-cli -m "DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf" -p "Who are you? Answer as LumynaX in two sentences." -n 160
```

**Ollama path:**

```bash
ollama create lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf -f ollama/Modelfile
ollama run lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf
```

**Verify integrity before launch:**

```bash
sha256sum "DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf"
cat checksums.sha256
```

```powershell
Get-FileHash -Algorithm SHA256 "DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf"
...
| Field | Value |
| --- | --- |
| Release | `LumynaX Reasoning DeepSeek-R1 Distill Llama 70B GGUF` |
| Repository | `AbteeXAILab/lumynax-reasoning-deepseek-r1-distill-llama-70b-gguf` |
| Family | `deepseek` |
...
| Field | Value |
| --- | --- |
| Primary | `DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf` |
| Weight size | `—` |
| Parameters | `70B` |
...
| Field | Value |
| --- | --- |
| Upstream / base | `deepseek-ai/DeepSeek-R1-Distill-Llama-70B` |
| Source | `DeepSeek R1 Distill` |
| License | `llama3.3` |
...
| Primary artifact | `DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf` &mdash; ships alongside manifest, checksums, quickstart, requirements, and license files. |
...
└── DeepSeek-R1-Distill-Llama-70B-Q4_K_M.gguf# primary artifact
```
...
```text
You are LumynaX operating from the LumynaX Reasoning DeepSeek-R1 Distill Llama 70B GGUF package identity. Be helpful, clear, and honest about provenance. Identify upstream models when asked. Do not invent biographical claims about named people without verified context.
```
